"""
Script Import: products.csv → Elasticsearch index 'products'  (versi baru pakai bulk)
Jalankan dengan: python scripts/import_products.py
Pastikan sudah install: pip install pandas elasticsearch
"""
import pandas as pd
from elasticsearch import Elasticsearch, helpers
import math

# Koneksi ke Elasticsearch
es = Elasticsearch("http://localhost:9200")
index_name = "products"

# Buat index dengan mapping jika belum ada
if not es.indices.exists(index=index_name):
    es.indices.create(index=index_name, body={
        "mappings": {
            "properties": {
                "product_id":   {"type": "keyword"},
                "product_name": {
                    "type": "text",
                    "fields": {
                        "keyword": {"type": "keyword"}
                    }
                },
                "category":     {
                    "type": "text",
                    "fields": {
                        "keyword": {"type": "keyword"}
                    }
                },
                "brand":        {
                    "type": "text",
                    "fields": {
                        "keyword": {"type": "keyword"}
                    }
                },
                "price":        {"type": "float"},
                "rating":       {"type": "float"}
            }
        }
    })
    print(f"Index '{index_name}' berhasil dibuat.")
else:
    print(f"Index '{index_name}' sudah ada, skip pembuatan mapping.")

# Baca CSV
df = pd.read_csv("dataset/ecommerce_dataset/products.csv")
df = df.where(pd.notnull(df), None)

# Bulk insert
def gen_docs(df):
    for i, row in df.iterrows():
        doc = row.to_dict()
        doc = {k: (None if (isinstance(v, float) and math.isnan(v)) else v) for k, v in doc.items()}
        yield {
            "_index": index_name,
            "_id": i + 1,
            "_source": doc
        }

helpers.bulk(es, gen_docs(df))
print(f"✅ Berhasil import {len(df)} dokumen ke index '{index_name}'")
